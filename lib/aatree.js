...
function Cursor (cmp, owner, path, key) {
  var found = path != null && path.branch === 0
  ...
  this.next = _next
  this.set = _set
  this.unset = this.remove = this['delete'] = _unset

  function _previous () {
    var path2 = previous (path)
...
function next (path) {
  ...
  if (path.branch < 0)
    return { branch:0, node:path.node, tail:path.tail }

  if (path.node.r === EMPTY) { 
