...
function Cursor (owner, path, key) {
  ...
  this._reify = function () { return path }
  ...
  function _unset () {
    ...
    return new AATree (owner.compare, unset (path), owner.size -1) }

  function _set (value) {
    const size = found ? owner.size : owner.size + 1
    return new AATree (owner.compare, set (path, key, value), size) }
}
...
// Exports
Object.defineProperty (AATree, 'core', { value: { Node, Empty:EMPTY } })
