...
const defaultCompare = function (a1, a2) {
...
class AATree {
  ...
  constructor (compare = defaultCompare, store = EMPTY) {
    ...
    Object.defineProperties (this, {
      store: { value: store, configurable: true },
...
AATree.defaultCompare = defaultCompare


// Cursor

function Cursor (owner, path, key) {
  const found = path != null && path.branch === 0
  this.__reify = function () { return path }

  this.found = found
  this.key = found ? path.node.key : key
  this.value = found ? path.node.value : this.value
  this.previous = this.prev = _previous
  this.next = _next
  this.set = _set
  this.unset = _unset
  ...
  function _previous () {
    ...
    return path2 ? new Cursor (owner, path2) : null }
  ...
  function _unset () {
    ...
    return new AATree (owner.compare, unset (path)) }

  function _set (value) {
    return new AATree (owner.compare, set (path, key, value)) }
}
...
function unset (path) {
  ...
  /* If n0 is not a leaf node, 
   it for swapping its value with
...
AATree._internal = { Node:Node, EMPTY:EMPTY }
