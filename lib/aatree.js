...
function select (k) {
  ...
  while (n !== EMPTY) {
    c = cmp(k, n.k)
    if (c === 0)
      return new Cursor (cmp, { node:n, branch:1, tail:p }, n.k)
    p = { branch:c, node:n, tail:p } // branch: -1 = left, 1 = right, >1 = after right. 
    n = (c < 0) ? n.l : n.r }
  return new Cursor (cmp, { node:n, branch:-1, tail:p }, k)
}
...
function Cursor (cmp, path, key) {
  this.found = (path.node !== EMPTY)
  this.key = (path.node !== EMPTY) ? path.node.k : key
  this.value = path.node.v
  this.previous = previous
  this.next = next
  this.set = _set
  this.unset = this.found ? _unset : null

  // TODO: add `previous`
  function previous () {
    var path2 = step (path, -1)
...
// Cursors are represented internally as a path of nodes,
// `path := null | { node, branch:-1|1, tail:path }`, representing a pointer
// at the left branch (-1), right branch (1), or when stepping through,
// before the left (<= 1) or after the right (>= 1) branch. 

function step (path, direction) {
  var d = direction < 0 ? -1 : 1 /* normalise */
  while (true) {
    var n = path.node
      , i = path.branch

    if (n !== EMPTY && i === -1)
      path = { node:n.l, branch:-d, tail:path }
    
    else if (n !== EMPTY && i === 1)
      path = { node:n.r, branch:-d, tail:path }

    else if (path.tail !== null) {
      var t = path.tail
      path = { node:t.node, branch:t.branch+(2*d), tail:t.tail }
      if (t.branch === -d) return path } /* emit inorder */
...
function unset (path) {
  ...
  while (p !== null) {
    ...
    // Decrease and rebalance, make sure to copy. 
