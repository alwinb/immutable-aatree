...
const _fromInternal = {}
const $$iterator = Symbol != null ? Symbol.iterator : '@@iterator'
...
function entries () {
  ...
  /* where*/
  function fn (p) {
    return { done: p == null, value: p && [p.key, p.value] } } }

function keys () {
  return stream (fn)
  /* where*/
  function fn (p) {
    return { done: p == null, value: p && p.key } } }

function values () {
  return stream (fn)
  /* where*/
  function fn (p) {
    return { done: p == null, value: p && p.value } } }


function stream (_fn) {
  _fn = typeof _fn !== 'function' ? function (x) { return x } : _fn
  let n = store
  const stack = []
  const r = { next: next }
  r[$$iterator] = () => r
  ...
  function next () {
    ...
    if (stack.length) {
      const n_ = stack.pop()
      n = n_.r
      return _fn ({ key:n_.k, value:n_.v }) }
    return _fn (null) } }
